...
export const changePassword = async (req, res) => {
  const { idusua, password } = req.body
  ...
  try {
    ...
    const { err, dat } = await usuario.changePassword()

    if (err) {
      res.status(401).json('No se ha podido cambiado la contraseña')
    } else {
      res.status(201).json('Se ha cambiado la contraseña')
    }
  } catch (error) {
    res.status(500).json('No se ha podido conectar con el servidor')
  }
