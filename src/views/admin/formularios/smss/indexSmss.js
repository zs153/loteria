...
const buildTable = (state) => {
  ...
  myList.map(element => {
    ...
    cell.innerHTML = `<div class="d-flex py-1 align-items-center">
      <div class="flex-fill">
        <div class="font-weight-medium">${element.FECSMS.slice(0,10).split("-").reverse().join("/")}</div>
      </div>
    </div>
    `
    row.appendChild(cell)
    ...
    cell.innerHTML = `<ul class="dots-menu">
      <li class="nav-item drop-right">
        ...
        <ul>
          <li class="nav-item">
            <a href="/admin/formularios/smss/edit/${element.IDFORM}/${element.IDSMSS}" class="nav-link">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
const createPages = () => {
  ...
  if (hasPrevs) {
    str += "<li class='page-item previous no'><a href='/admin/formularios/smss?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarSmsBox').value + "&dir=prev' class='nav-link'>&#9664 Anterior</a>";
  } else {
    str += "<li><a href='#' class='nav-link disabled'>&#9664 Anterior</a>";
  }

  if (hasNexts) {
    str += "<li class='page-item next no'><a href='/admin/formularios/smss?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarSmsBox').value + "&dir=next' class='nav-link'>Siguiente &#9654</a>";
  } else {
...
elemDel.setAttribute('action', `/admin/formularios/smss/delete?part=${getCookie('filtro')}`)
