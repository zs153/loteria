  ...
  myList.map(element => {
    ...
    cell.innerHTML = `<ul class="dots-menu">
      <li class="nav-item drop-right p-0">
        ...
        <ul>
          <li class="nav-item">
            <a href="/admin/tipos/edit/${element.IDTIPO}" class="nav-link">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
          ...
          <li class="nav-item">
            <a href="#" class="nav-link" onclick="{document.getElementById('idtipo').value ='${element.IDTIPO}', document.getElementById('msgbor').innerHTML ='<p>${element.DESTIP}</p>'}" data-bs-toggle="modal" data-bs-target="#modal-borrar">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
const createPages = () => {
  ...
  if (hasPrevs) {
    str += "<li class='page-item previous no'><a href='/admin/tipos?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarTipoBox').value + "&dir=prev' class='nav-link'>&#9664 Anterior</a>";
  } else {
    str += "<li><a href='#' class='nav-link disabled'>&#9664 Anterior</a>";
  }

  if (hasNexts) {
    str += "<li class='page-item next no'><a href='/admin/tipos?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarTipoBox').value + "&dir=next' class='nav-link'>Siguiente &#9654</a>";
  } else {
