...
const buildTable = (state) => {
  ...
  myList.map(element => {
    ...
    cell = document.createElement('td')
    cell.classList.add("w-5")
    if (element.STAFOR === estadosFormulario.resuelto) {
...
const createPages = () => {
  ...
  if (hasPrevFras) {
    str += "<li class='page-item previous no'><a href='/user/formularios/resueltos?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarFormularioBox').value + "&dir=prev' class='nav-link'>&#9664 Anterior</a>";
  } else {
    str += "<li><a href='#' class='nav-link disabled'>&#9664 Anterior</a>";
  }

  if (hasNextFras) {
    str += "<li class='page-item next no'><a href='/user/formularios/resueltos?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarFormularioBox').value + "&dir=next' class='nav-link'>Siguiente &#9654</a>";
  } else {
...
// events
const elemBuscar = document.getElementById('buscarFormularioBox');
...
elemPen.setAttribute('href', `/user/formularios?part=${getCookie('filtro')}`)
